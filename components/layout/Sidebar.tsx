...
export default function Sidebar() {
  ...
  const displayName = profile?.full_name
    ? capitalize(profile.full_name)
    : user?.email
      ? capitalize(user.email.split('@')[0].replace(/[._]/g, ' '))
      : 'User'
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(!mobileOpen)} className="sidebar-mobile-toggle" aria-label="Toggle menu">
      ...
      <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
        ...
        <nav style={{ flex: 1, padding: '4px 0' }}>
          {nav.map((n) => (
            <Link key={n.href} href={n.href} onClick={closeMobile}
