...
type Row = {
  id: string
  stage: string
  candidates: { id: string; name: string; current_title: string | null; current_company: string | null } | null
}
...
export default function KanbanBoard({
  jobId, initialRows, currentProfile
}: {
  jobId: string
  initialRows: Row[]
  ...
  async function moveStage(row: Row, direction: 'left' | 'right') {
    ...
    await (supabase.from('pipeline') as any).update({ stage: newStage }).eq('id', row.id)
        ...
        return (
          <div key={stage}
            ...
            <div className={`px-3 py-2.5 border-b border-gray-100 ${stageHeaderBg(stage)}`}>
              <p className={`text-xs font-semibold leading-tight ${stageColor(stage)}`}>{stage}</p>
              <p className="text-xs text-gray-400 mt-0.5">{cards.length} candidate{cards.length !== 1 ? 's' : ''}</p>
            </div>
            <div className="flex flex-col gap-2 p-2 flex-1">
              {cards.map((row, i) => {
                ...
                return (
                  <div key={row.id}
                    ...
                    {c.current_title && (
                      <p className="text-xs text-gray-400 mb-2 leading-tight">{c.current_title}</p>
                    )}
              ...
              {isLast && cards.length > 0 && (
                <div className="mt-1 text-center">
                  <span className="text-xs text-emerald-600 font-medium">🎉 Invoice time!</span>
                </div>
              )}
            </div>
          </div>
